...
def compute_conditional_distributions(
    ...
    **kwargs
    ...
    """
    Compute the conditional distribution of target variables given key variables,
    P(TARGET | KEY), for two datasets.

    Args:
        data1 (pd.DataFrame):
            Baseline dataset used to compute the reference conditional distribution.

        data2 (pd.DataFrame):
            Dataset evaluated or compared against the baseline.
        ...
        k_uniques:
            Precomputed unique key combinations.

        t_uniques:
            Precomputed unique target combinations.
    ...
    Keyword Args:
        neighborhood (int):
            Maximum Hamming distance used for neighborhood approximation.
            Default: 1.

    Returns:
        cond_dist1 (pd.DataFrame):
            Conditional distribution of data1.

        cond_dist2 (pd.DataFrame):
            Conditional distribution of data2.
    """
    
    # =============================================================================================
    # 1. Prepare Key and Target Combinations
    # =============================================================================================
    ...
    if k_uniques is None and t_uniques is None:
        ...
        key_tuples = pd.MultiIndex.from_frame(
            concat_data[key_cols]
        ).to_list()
        
        target_tuples = pd.MultiIndex.from_frame(
            concat_data[target_cols]
        ).to_list()
        
        _, k_uniques = pd.factorize(
            pd.Index(key_tuples), 
            sort=True
        )
        
        _, t_uniques = pd.factorize(
            pd.Index(target_tuples), 
            sort=True
        )
    ...
    # =============================================================================================
    # 2. Encode Key and Target Combinations
    # =============================================================================================
    
    k_codes1 = k_uniques.get_indexer(
        pd.MultiIndex.from_frame(data1[key_cols])
    )
    t_codes1 = t_uniques.get_indexer(
        pd.MultiIndex.from_frame(data1[target_cols])
    )
    k_codes2 = k_uniques.get_indexer(
        pd.MultiIndex.from_frame(data2[key_cols])
    )
    t_codes2 = t_uniques.get_indexer(
        pd.MultiIndex.from_frame(data2[target_cols])
    )
    ...
    unmatched_keys_idx = np.where(
        row_sums2.ravel() == 0
    )
    
    cond_prob1 = np.divide(
        counts1, 
        row_sums1, 
        out = result1, 
        where=row_sums1 != 0
    ...
    # =============================================================================================
    # 4. Construct Composite Keys and Targets
    # =============================================================================================
    ...
    # =============================================================================================
    # 5. Imputation for Unmatched Keys
    # =============================================================================================
    ...
    if imputation in ["marginal", "neighborhood_appr"]:
        ...
        # -----------------------------------------------------------------------------------------
        # 5-2. Neighborhood Approximation
        # -----------------------------------------------------------------------------------------
            ...
            if neighborhood is None:
                neighborhood = 1
                warnings.warn(
                    "For 'appr imputation', a 'neighborhood' value was not provided. "
